...
export class CashLoansService {
  ...
  private async resolveCompanyTimezone(company_id: string) {
    ...
    return {
      tz: company?.timezone ?? 'Asia/Riyadh',
      slug: company?.slug ?? company_id.slice(0, 8),
    };
  }
  ...
  private getMonthBounds(now: Date, timeZone: string): DateRange {
    ...
    const end = new Date(
      Date.UTC(year, month - 1, Number(parts.day), 23, 59, 59, 999),
    );
  ...
  private ensureCurrentMonth(dateStr: string, timeZone: string) {
    ...
    if (isNaN(parsed.getTime()))
      throw new BadRequestException('FIN_CASH_001: Invalid date');
    const now = new Date();
    const { from, to } = this.getMonthBounds(now, timeZone);
    if (parsed.getTime() < from.getTime() || parsed.getTime() > to.getTime()) {
      throw new BadRequestException(
        'FIN_CASH_002: Date must be within the current month and not in the future',
      );
    }
    return parsed;
  }
  ...
  private async ensureActiveEmployment(
    ...
    if (!employment)
      throw new BadRequestException(
        'FIN_CASH_003: Employment record must be active',
      );
    return employment;
  }
  ...
  private async verifyFile(company_id: string, file_id?: string | null) {
    ...
    if (!file)
      throw new BadRequestException(
        'FIN_CASH_004: Attachment not found for this company',
      );
    return file.id;
  }
  ...
  private async applyWalletDelta(
    ...
    if (newBalance.lt(0))
      throw new BadRequestException(
        'FIN_CASH_005: Wallet balance cannot be negative',
      );
      ...
      where: {
        ...
        type: {
          in: [
            ...
            CashTransactionType.LOAN,
            CashTransactionType.DEDUCTION,
          ],
        },
      },
      _sum: { amount: true },
  ...
  async stats(
    ...
    try {
      this.logger.debug(
        `Computing stats for company ${company_id}, user ${user_id}`,
        { input },
      );
      ...
      this.logger.debug(`Normalized range:`, {
        from: range.from,
        to: range.to,
      });
      ...
      const notCollectedAgg = await this.computeNotCollectedTotal(
        company_id,
        range,
      );
  ...
  private normalizeRange(
    ...
    const fromCandidate = input.date_from
  ...
  private async computeNotCollectedTotal(company_id: string, range: DateRange) {
    ...
    const receiptMap = new Map<
      string,
      { receipt: Prisma.Decimal; deduction: Prisma.Decimal }
    >();
    for (const row of receiptsAgg) {
      ...
      if (row.type === CashTransactionType.RECEIPT)
        entry.receipt = entry.receipt.plus(row._sum.amount ?? 0);
      if (row.type === CashTransactionType.DEDUCTION)
        entry.deduction = entry.deduction.plus(row._sum.amount ?? 0);
  ...
  private async computeEmployeeDue(
    company_id: string,
    employment_record_id: string,
    range: DateRange,
  ) {
  ...
  async listEmployees(
    ...
    txnAgg.forEach(
      (row: {
        ...
        const entry = txnMap.get(row.employment_record_id) ?? {
          receipt: new Prisma.Decimal(0),
          loan: new Prisma.Decimal(0),
          deduction: new Prisma.Decimal(0),
        };
        if (row.type === CashTransactionType.RECEIPT)
          entry.receipt = entry.receipt.plus(row._sum.amount ?? 0);
        if (row.type === CashTransactionType.LOAN)
          entry.loan = entry.loan.plus(row._sum.amount ?? 0);
        if (row.type === CashTransactionType.DEDUCTION)
          entry.deduction = entry.deduction.plus(row._sum.amount ?? 0);
    ...
    const computed = employees.map((emp) => {
      ...
      const totalCashCollected = new Prisma.Decimal(
        ops?._sum?.cash_collected ?? 0,
      );
      const loans = tx?.loan ?? new Prisma.Decimal(0);
      const receipts = tx?.receipt ?? new Prisma.Decimal(0);
      const deductions = (tx?.deduction ?? new Prisma.Decimal(0)).plus(
        new Prisma.Decimal(ops?._sum?.deduction_amount ?? 0),
      );
  ...
  async employeeDetail(
    ...
    if (!employment)
      throw new NotFoundException('FIN_CASH_006: Employee not found');
    ...
    const receipts = txns
      ...
      .reduce(
        (acc: Prisma.Decimal, t) => acc.plus(t.amount),
  ...
  async createReceipt(
    ...
    const attachment = await this.verifyFile(
      company_id,
      input.attachment_file_id,
    );

    const amount = Number(input.amount ?? 0);
    if (amount <= 0)
      throw new BadRequestException('FIN_CASH_007: Amount must be positive');
    const isDraft = input.submit_action === 'draft';
    if (!isDraft && !attachment)
      throw new BadRequestException(
        'FIN_CASH_008: Attachment required for approval',
      );
    const monthRange = this.getMonthToDateRange(date, tz);
    const due = await this.computeEmployeeDue(
      company_id,
      employment.id,
      monthRange,
    );
    if (!isDraft && new Prisma.Decimal(amount).gt(due)) {
      throw new BadRequestException(
        'FIN_CASH_021: Amount exceeds outstanding due',
      );
    }
    ...
    return this.prisma.$transaction(async (trx) => {
      ...
      await this.audit.log({
        ...
        actor_user_id,
        action: isDraft
          ? 'FIN_CASH_RECEIPT_DRAFT'
          : 'FIN_CASH_RECEIPT_APPROVED',
  ...
  async createLoan(
    ...
    if (amount <= 0)
      throw new BadRequestException(
        'FIN_CASH_009: Loan amount must be positive',
      );
    const isDraft = input.submit_action === 'draft';
    ...
    return this.prisma.$transaction(async (trx) => {
      ...
      if (!isDraft) {
        ...
        if (newBalance.lt(0))
          throw new BadRequestException(
            'FIN_CASH_010: Loan exceeds supervisor wallet balance',
          );
        balance_after = newBalance;
        await trx.walletBalance.update({
          where: { company_id_user_id: { company_id, user_id: actor_user_id } },
  ...
  async createDeduction(
    ...
    if (amount <= 0)
      throw new BadRequestException(
        'FIN_CASH_011: Deduction amount must be positive',
      );
    const isDraft = input.submit_action === 'draft';
  ...
  async updateTransactionStatus(
    ...
    const existing = await this.prisma.cashTransaction.findFirst({
      where: { id, company_id },
    });
    if (!existing)
      throw new NotFoundException('FIN_CASH_012: Transaction not found');
    if (existing.status !== CashTransactionStatus.DRAFT)
      throw new BadRequestException('FIN_CASH_013: Only drafts can be updated');

    const attachment = await this.verifyFile(
      company_id,
      attachment_file_id ?? existing.attachment_file_id,
    );
    ...
    return this.prisma.$transaction(async (trx) => {
      ...
      if (!receipt_no) {
        const code =
          existing.type === CashTransactionType.LOAN
            ? 'CASH_LOAN_SEQ'
            : existing.type === CashTransactionType.DEDUCTION
              ? 'CASH_DED_SEQ'
              : 'CASH_RECEIPT_SEQ';
        const prefix =
          existing.type === CashTransactionType.LOAN
        ...
        const counter = await trx.usageCounter.upsert({
          where: {
            company_id_counter_code: { company_id, counter_code: code },
          },
          update: { value: { increment: 1 } },
      ...
      if (
        existing.type === CashTransactionType.RECEIPT ||
        existing.type === CashTransactionType.DEDUCTION
      ) {
        const wallet = await this.ensureWallet(
          company_id,
      ...
      if (existing.type === CashTransactionType.LOAN) {
        ...
        if (newBalance.lt(0))
          throw new BadRequestException(
            'FIN_CASH_014: Loan exceeds supervisor wallet balance',
          );
        balance_after = newBalance;
        await trx.walletBalance.update({
          where: { company_id_user_id: { company_id, user_id: actor_user_id } },
  ...
  async handover(
    ...
    if (!Array.isArray(input.expenses) || input.expenses.length === 0) {
      throw new BadRequestException(
        'FIN_CASH_015: At least one expense is required',
      );
    }
    ...
    for (const row of input.expenses) {
      ...
      if (amount <= 0)
        throw new BadRequestException(
          'FIN_CASH_016: Expense amount must be positive',
        );
      const statement = (row.statement ?? '').trim();
      if (statement.length < 2)
        throw new BadRequestException(
          'FIN_CASH_017: Expense statement required',
        );
      const fileId = await this.verifyFile(company_id, row.receipt_file_id);
      expensesValidated.push({
        statement,
        amount: new Prisma.Decimal(amount),
        receipt_file_id: fileId ?? null,
      });
    }
    ...
    return this.prisma.$transaction(async (trx) => {
      ...
      const expensesTotal = expensesValidated.reduce(
        (acc: Prisma.Decimal, e) => acc.plus(e.amount),
        new Prisma.Decimal(0),
      );
      const handedOverAmount = new Prisma.Decimal(wallet.balance).minus(
        expensesTotal,
      );
      if (handedOverAmount.lt(0))
        throw new BadRequestException(
          'FIN_CASH_018: Expenses exceed wallet balance',
        );
      if (
        !isDraft &&
        !handedOverAmount.plus(expensesTotal).equals(wallet.balance)
      ) {
        throw new BadRequestException(
          'FIN_CASH_019: Handover totals mismatch wallet balance',
        );
      }
      ...
      const batch = await trx.handoverBatch.create({
        data: {
          ...
          supervisor_user_id: actor_user_id,
          status: isDraft
            ? CashHandoverStatus.DRAFT
            : CashHandoverStatus.APPROVED,
          date,
      ...
      if (!isDraft) {
        const newBalance = new Prisma.Decimal(wallet.balance)
          .minus(expensesTotal)
          .minus(handedOverAmount);
        if (newBalance.lt(0))
          throw new BadRequestException(
            'FIN_CASH_020: Handover would create negative wallet balance',
          );
        await trx.walletBalance.update({
          where: { company_id_user_id: { company_id, user_id: actor_user_id } },
          data: { balance: newBalance, updated_at: new Date() },
      ...
      await this.audit.log({
        ...
        actor_user_id,
        action: isDraft
          ? 'FIN_CASH_HANDOVER_DRAFT'
          : 'FIN_CASH_HANDOVER_APPROVED',
